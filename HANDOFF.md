...
- **미결 2건**
  1. `tools/cgc-pop.js` 가 목록 1페이지만 읽는다. CGC 목록이 2페이지로 늘어(2026-08-03 확인) 그대로 두면
     일본판 7세트(OP-01·06·08·10·14·16·PRB-02)가 매주 조용히 누락된다. 이번엔 `?page=2` 를 손으로 읽어 합쳤다.
  2. 주간 워크플로(`update-market-data.yml`, 일 20:00 UTC)가 안 돈 것으로 보인다 — PSA 가 7/26 에 멈춰 있었고
     8/3 05:00 KST 실행 흔적이 없다. 이 워크플로는 그레이딩 말고 **eBay 팩 가격·영문 NM·박스 시계열·감사**도 하므로
     별개로 확인이 필요하다(일일 워크플로 `update-active-listings` 는 정상 동작 중).
