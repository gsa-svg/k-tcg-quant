...
- GemRate 공개 세트 추이 21개를 동일 기준으로 검증해 `data/gemrate-psa-history.json`에 최소 필요 데이터만 보존했다. 최신 누적은 2026-07-20, 주간 막대는 2026-07-15까지다.
- 기존 `psaFull`은 일부 대표 카드 합계를 세트 전체처럼 표시한 정합성 오류가 있었다. 전 세트를 실제 전체 세트 누적 등급·PSA10 수·젬률로 교체했다(21세트 합계 577,660장).
- 2026-06-10 누적 감소와 06-17 복구 급증은 원본 정정 이상 구간이라 제외했다. 그래프는 동일 기준으로 정상 연결되는 06-24, 07-01, 07-08, 07-15 네 주만 사용한다.
- `tools/import-gemrate-psa-history.js`가 21세트 커버리지·날짜·누적값·주간 증분을 검증한 뒤 데이터를 반영한다. `tools/test-import-gemrate-psa-history.js`로 회귀 방지한다.
- 작은 신규 세트(OP-16 등)의 막대가 거의 안 보이던 고정 1,000축을 동적 축으로 교정했다. PSA 패널 문구도 Top 10과 전체 세트가 섞이지 않게 수정했다.
- 공개 페이지는 브라우저에서는 확인되지만 서버 직접 요청은 차단된다. 승인된 API 없이 주간 자동 스크래핑으로 가장하지 말 것. 다음 갱신은 같은 21개 URL을 다시 검증해 source JSON을 갱신한 뒤 importer를 실행한다.
