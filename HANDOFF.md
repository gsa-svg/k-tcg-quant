...
- **페이지네이션 누락 대책 (2026-08-03 적용, 커밋 참조)** — 위 1번은 **해결**.
  ...
  - `box-sold-urls.js`: 검색 240건 상한에 걸렸는지 `rawN`/`capped` 로 보고(`_sop=13` 이라 잘리는 건 오래된 쪽, 주3회라 손실 없음).
  - 자체 테스트 `tools/test-pop-ingest-guards.js`(4케이스).
- **미결 1건**
  1. 주간 워크플로(`update-market-data.yml`, 일 20:00 UTC)가 안 돈 것으로 보인다 — PSA 가 7/26 에 멈춰 있었고
