...
```powershell
& 'C:\Users\kimtt\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe' .\tools\test-price-outliers.js
& 'C:\Users\kimtt\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe' --check .\tools\update-ebay-psa10-prices.js
& 'C:\Users\kimtt\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe' --check .\packs.js
```
...
- `PSA10 eBay` 수집기는 표본이 2개일 때 한쪽 가격이 5배 이상 튀고 차이가 $1,000을 넘으면 이상치로 제외한다. 표본이 3개 이상이면 중앙값 기준 3배 초과 고가와 1/5 미만 저가를 보수적으로 제외하고, 최고가가 바로 아래 값의 2배 이상이며 차이가 $1,000을 넘는 경우도 제외한다.
