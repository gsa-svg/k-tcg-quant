// ⛔ 2026-08-13 폐기 — tools/build-box-sold-series.js 로 대체됨. 실행하지 말 것.
//
// 왜 버렸나: 이 도구는 "수집일 스냅샷"을 하루 한 점씩 덧붙였다. 두 가지가 틀어져 있었다.
//  1) 수집 방식이 바뀌면 값이 튄다. 8/13 에 언어 판별을 eBay 신고값으로 바꾸자 일본판 표본이
//     10배로 늘었고 그날 점만 OP-13 $118 → $137 로 뛰었다. 시장이 아니라 우리가 바뀐 것이다.
//  2) 후행한다. 그날 검색에 보인 sold 를 전부 뭉갠 값이라 사실상 최근 90일 평균이었다
//     (OP-01 영문판 스냅샷 $1,525 vs 최근 4주 실거래 $1,363).
// 새 도구는 원장에서 **판매일** 기준으로 매번 다시 센다. 아래 코드는 이력으로만 남긴다.
