...
function hasVariantSignal(title, card) {
  ...
  // SP는 별도 변형 — 일반 parallel 판매건이 섞이지 않게 SP/special 명시 요구 + 프리미엄 배제 (speci4l = leetspeak 회피 방지)
  if (/\bsp\b|speci[a4]l/i.test(name)) return /\bsp\b|speci[a4]l/i.test(title) && !premiumTitle.test(title);
  // 일반 패러렐/알트아트 — 프리미엄·SP·Special 판매건이 저가 행에 붙지 않게 배제
  if (/parallel|alternate/i.test(name))
    return /parallel|alternate|alt\s*art/i.test(title) && !premiumTitle.test(title) && !/\bsp\b|speci[a4]l|red\s*text/i.test(title);
  return true;
}
