...
function renderBoxSeries(set) {
  ...
  return `<div class="boxChart"><div class="bcHead"><span class="bmLabel">${t("박스 시세 추이 · 최근 3개월 (eBay Sold)", "Box price trend · last 3 months (eBay Sold)")}</span><strong>${triMain(last.p, "KRW").main} <small style="font-weight:400;opacity:.7">${triMain(last.p, "KRW").sub}</small></strong></div><svg viewBox="0 0 ${W} ${H}" class="bcSvg" role="img" aria-label="${t("박스 시세 추이 그래프", "Box price trend chart")}"><path d="${area}" class="bcArea"></path><polyline points="${coords.join(" ")}" class="bcLine"></polyline>${coords.map((xy) => { const [x, y] = xy.split(","); return `<circle cx="${x}" cy="${y}" r="3.5" class="bcDot"></circle>`; }).join("")}</svg><div class="bcAxis"><span>${fmtD(pts[0].d)}</span><span>${t("최고", "High")} ${triMain(Math.max(...ys), "KRW").main} · ${t("최저", "Low")} ${triMain(Math.min(...ys), "KRW").main}</span><span>${fmtD(last.d)}</span></div><p class="note">${t("주간 중앙값입니다. 표본이 적은 주는 변동이 큽니다. 단일 박스 기준입니다.", "Weekly medians; weeks with few samples swing more. Single box only.")}</p></div>`;
}
...
function historyChart(history, market) {
  ...
  if (points.length < 2) return `<div class="cardChartEmpty">${t("3개월 그래프는 eBay NM 업데이트가 2회 이상 쌓이면 표시됩니다.", "The 3-month chart appears after at least two eBay NM updates.")}</div>`;
