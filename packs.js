...
function renderPsaDestruction(set) {
  ...
  return `<div class="boxChart psaDest"><div class="bcHead"><span class="bmLabel">${t("PSA 등급 · 개봉 현황", "Grading & Destruction")}</span><small class="pdSrc">${t("PSA 인구조사 · 세트 전체", "Population via PSA · full set")}${updated ? ` · ${updated}` : ""}</small></div>
    <div class="pdStats"><div class="pdStat"><span>${t("젬 (PSA 10)", "Total Gems (PSA 10)")}</span><b class="pdGemBig">${full ? num(full.gem10) : "-"}</b></div><div class="pdStat"><span>${t("누적 등급 수", "Total Grades")}</span><b>${full ? num(full.total) : (total != null ? num(total) : "-")}</b></div><div class="pdStat"><span>${t("젬 비율", "Gem Rate")}</span><b class="pdGemBig">${full ? full.gemRate + "%" : (gem != null ? gem + "%" : "-")}</b>${full && full.opAvg != null ? `<small style="display:block;color:#7d8698;font-size:11px;margin-top:3px;">${t("OP 평균", "OP avg")} ${full.opAvg}%${full.opDiff != null ? ` <em style="font-style:normal;color:${full.opDiff < 0 ? "#ff7a7a" : "#55d8ea"};">${full.opDiff > 0 ? "+" : ""}${full.opDiff}%</em>` : ""}</small>` : ""}</div></div>
    <p class="note" style="margin:7px 0 2px;">${t("위 3개는 <b>PSA 인구조사 세트 전체</b> 집계예요. 아래 막대는 <b>우리가 주별로 모은 등급 추이</b>라 흐름을 볼 수 있어요.", "The three figures above are <b>full-set PSA population totals</b>. The bars below are <b>our week-by-week trend</b> so you can watch the flow.")}</p>
