...
function renderBoxSeries(set) {
  ...
  return `<div class="boxChart"><div class="bcHead"><span class="bmLabel">${t("?? ?? ?? ? ?? 3?? (eBay Sold)", "Box price trend ? last 3 months (eBay Sold)")}</span><strong>${triMain(last.p, "KRW").main} <small style="font-weight:400;opacity:.7">${triMain(last.p, "KRW").sub}</small></strong></div><svg viewBox="0 0 ${W} ${H}" class="bcSvg" role="img" aria-label="${t("?? ?? ?? ???", "Box price trend chart")}"><path d="${area}" class="bcArea"></path><polyline points="${coords.join(" ")}" class="bcLine"></polyline>${coords.map((xy) => { const [x, y] = xy.split(","); return `<circle cx="${x}" cy="${y}" r="3.5" class="bcDot"></circle>`; }).join("")}</svg><div class="bcAxis"><span>${fmtD(pts[0].d)}</span><span>${t("??", "High")} ${triMain(Math.max(...ys), "KRW").main} ? ${t("??", "Low")} ${triMain(Math.min(...ys), "KRW").main}</span><span>${fmtD(last.d)}</span></div><p class="note">${t("?? ?????, ??? ?? ?? ??? ???. ?? ?? ?????.", "Weekly medians; weeks with few samples swing more. Single box only.")}</p></div>`;
}
