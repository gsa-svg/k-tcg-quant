...
function renderPsaDestruction(set) {
  ...
  return `<div class="boxChart psaDest"><div class="bcHead"><span class="bmLabel">${t("PSA 등급 · 개봉 현황", "Grading & Destruction")}</span><small class="pdSrc">${t("PSA 인구조사 · Top 10 카드", "Population via PSA · Top 10 cards")}${updated ? ` · ${updated}` : ""}</small></div>
    <p class="note" style="margin:3px 0 9px;">${t("아래 수치는 이 세트에서 시세가 가장 높은 <b>Top 10 카드</b>의 PSA 등급 기준이에요 — 세트 전체가 아니라 대표 상위 10장 합계입니다.", "These figures are for this set's <b>Top 10 cards</b> (highest market value) — the sum of their PSA grades, not the whole set.")}</p>
    <div class="pdStats"><div class="pdStat"><span>${t("Top 10 누적 등급", "Total graded · Top 10")}</span><b>${total != null ? num(total) : "-"}</b></div><div class="pdStat"><span>${t("젬 비율 · PSA10", "Gem rate · PSA10")}</span><b class="pdGemBig">${gem != null ? gem + "%" : "-"}</b></div><div class="pdStat"><span>${t("추적 카드", "Tracked cards")}</span><b>${rows.length}</b></div></div>
